...
- 期权（Option）：给予买方“选择权”，不是必须交易。你花一点钱，换取未来是否买入或卖出的权利。如果价格朝你有利的方向走，你就执行；否则你可以放弃。
...
因为它们把“未来可能发生的事”变成了一个可以被精确计算、被交易、被对冲的合同。你可以把它们理解成一套“把不确定性包装成可管理对象”的工具箱。
...
如果你把这三个问题弄清楚，后面的定价、波动率、保证金、对冲逻辑就会顺着自然展开。
