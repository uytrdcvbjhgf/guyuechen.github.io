...
- 最大损失为 9 刀，发生在标的到期正好为 100 刀时；
- 上方盈亏平衡点为 100 + 9 = 109 刀；
- 下方盈亏平衡点为 100 - 9 = 91 刀；
...
- 标的位于 95 到 105 刀：两份期权都失效，损失全部 5 刀；
- 标的高于 105 刀：Call 开始产生内在价值，超过 110 刀后整体盈利；
- 标的低于 95 刀：Put 开始产生内在价值，跌破 90 刀后整体盈利。

与 Straddle 相比，Strangle 少支付了 4 刀，但上、下盈亏平衡点反而从 109 / 91 推远到 110 / 90。便宜并没有消灭成本，只是把成本换成了对更大价格移动的要求。
...
- 标的位于 95 到 105 刀：四条腿都没有内在价值，保留全部 2 刀 credit；
- 标的低于 95 或高于 105 刀：组合开始回吐 credit；
- 标的低于 90 或高于 110 刀：一侧垂直价差达到完整宽度，损失不再扩大。

在左右翼宽度都为 5 刀的例子中：

> 最大收益 = Net Credit = 2 刀

> 最大损失 = Wing Width - Net Credit = 5 - 2 = 3 刀

> 下方盈亏平衡点 = Short Put Strike - Credit = 95 - 2 = 93 刀

> 上方盈亏平衡点 = Short Call Strike + Credit = 105 + 2 = 107 刀
...
- 标的不高于 95 刀：所有 Call 都失效，损失 2 刀 debit；
- 标的从 95 向 100 刀靠近：组合价值逐渐上升；
- 标的正好为 100 刀：组合价值为 5 刀，扣除 debit 后最大收益为 3 刀；
- 标的从 100 向 105 刀移动：组合价值逐渐下降；
- 标的不低于 105 刀：三组 Call 的 Delta 相互抵消，组合到期价值回到 0，损失 2 刀 debit。

所以：

> 最大收益 = Wing Width - Net Debit = 5 - 2 = 3 刀
...
Short Iron Condor 的最大收益是一个区间：本文例子中，只要到期位于 95 到 105 刀，结果都是 +2 刀。Long Butterfly 的最大收益只有一个点：越接近 100 刀越好，偏离 body 后收益会线性减少。
...
例如，理论上可收到 2 刀的 Iron Condor，如果实际只能按 1.70 刀成交，那么最大收益减少到 1.70 刀，最大损失增加到 3.30 刀，上下盈亏平衡点也会一起向内移动。
