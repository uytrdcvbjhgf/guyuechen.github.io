...
- 标的现价为 100 刀；
- 95 Call 权利金为 7.50 刀；
- 105 Call 权利金为 2.50 刀；
- 95 Put 权利金为 2.50 刀；
- 105 Put 权利金为 7.50 刀；
- 每份期权暂按 1 单位标的计算；
- 忽略利率、股息、手续费、税费和买卖价差。

因此，四种组合的净 debit 或 credit 都恰好为 5 刀，盈亏平衡点都落在 100 刀。这种对称是为了看清结构，真实报价不会总是如此整齐。
...
> 买入 95 Call，支付 7.50 刀
>
> 卖出 105 Call，收入 2.50 刀
>
> 初始净支出 = 5 刀

Long 95 Call 提供上涨收益，Short 105 Call 补贴 2.50 刀成本，却也让 105 刀以上的收益不再增加。

到期结果可以分成三段：

- 标的不高于 95 刀：两份 Call 都失效，最大损失为净支出 5 刀；
- 标的位于 95 到 105 刀：只有 Long Call 产生内在价值，盈亏随标的上涨；
- 标的不低于 105 刀：两份 Call 的价差固定为 10 刀，扣除净支出后最大收益为 5 刀。

因此：

> 最大损失 = Net Debit = 5 刀

> 最大收益 = Spread Width - Net Debit = 10 - 5 = 5 刀

> 到期盈亏平衡点 = Long Call Strike + Net Debit = 95 + 5 = 100 刀

与单独买入 95 Call 相比，Bull Call 少支付了 2.50 刀，但放弃了 105 刀以上的上涨收益。Short Call strike 本质上是给自己的上涨预测设置目标价：超过这个位置继续看对，也不会继续增加到期利润。

## Bull Put Spread：收到 credit，并承担有限下跌风险

Bull Put Spread 使用相同的两个 strike，但换成 Put：

> 卖出 105 Put，收入 7.50 刀
>
> 买入 95 Put，支付 2.50 刀
>
> 初始净收入 = 5 刀
...
- 标的不低于 105 刀：两份 Put 都失效，保留全部 5 刀 credit；
- 标的位于 95 到 105 刀：Short Put 逐渐产生亏损，Long Put 仍无内在价值；
- 标的不高于 95 刀：两份 Put 的价差固定为 10 刀，扣除已收 credit 后最大损失为 5 刀。

因此：

> 最大收益 = Net Credit = 5 刀

> 最大损失 = Spread Width - Net Credit = 10 - 5 = 5 刀

> 到期盈亏平衡点 = Short Put Strike - Net Credit = 105 - 5 = 100 刀

“开仓收到 5 刀”不表示交易已经赚到 5 刀。这笔 credit 是承担 105 Put 义务的补偿，只有义务消失或组合平仓后，才能确认最终盈亏。

## Bear Put Spread：降低 Long Put 成本，同时封住下方

Bear Put Spread 是 Bull Call 的向下版本：

> 买入 105 Put，支付 7.50 刀
>
> 卖出 95 Put，收入 2.50 刀
>
> 初始净支出 = 5 刀

Long 105 Put 从下跌中获利，Short 95 Put 补贴成本，并把 95 刀以下的继续下跌收益封住。

到期结果为：

- 标的不低于 105 刀：两份 Put 都失效，最大损失为净支出 5 刀；
- 标的位于 95 到 105 刀：Long Put 产生内在价值，盈亏随标的下跌改善；
- 标的不高于 95 刀：两份 Put 的价差固定为 10 刀，最大收益为 5 刀。

对应公式是：

> 最大损失 = Net Debit = 5 刀

> 最大收益 = Spread Width - Net Debit = 5 刀

> 到期盈亏平衡点 = Long Put Strike - Net Debit = 105 - 5 = 100 刀

如果认为标的会下跌，但不认为它会跌得远低于 95 刀，卖出 95 Put 可以减少 Long Put 的初始成本。代价是即使标的跌到 70 刀，组合最大收益仍停在 5 刀。

## Bear Call Spread：收到 credit，并承担有限上涨风险

Bear Call Spread 由一空一多两份 Call 组成：

> 卖出 95 Call，收入 7.50 刀
>
> 买入 105 Call，支付 2.50 刀
>
> 初始净收入 = 5 刀
...
- 标的不高于 95 刀：两份 Call 都失效，最大收益为 5 刀 credit；
- 标的位于 95 到 105 刀：Short Call 逐渐产生亏损；
- 标的不低于 105 刀：两份 Call 的价差固定为 10 刀，最大损失为 5 刀。

因此：

> 最大收益 = Net Credit = 5 刀

> 最大损失 = Spread Width - Net Credit = 5 刀

> 到期盈亏平衡点 = Short Call Strike + Net Credit = 95 + 5 = 100 刀
...
本文中 debit 与 credit 都是 5 刀，宽度为 10 刀，因此方向相同的组合拥有相同的最大收益、最大损失和盈亏平衡点。
...
把 95 / 105 的宽度从 10 刀扩大到 20 刀，通常会扩大最大潜在收益和损失，但不能只看到“defined risk”就忽略账户金额。

如果合约乘数为 100，本文 5 刀的最大损失对应：

> 5 × 100 = 500 刀 / 组

交易 10 组就变成 5,000 刀。宽度、净权利金、乘数和组合数量必须一起换算。
